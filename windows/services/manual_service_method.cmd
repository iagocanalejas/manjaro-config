...
if %ERRORLEVEL% EQU 1 ( 
	echo %ESC%[46m[ %1 Service Already Set to Manual ]%ESC%[0m  
) else (
	sc config %1 start= manual
	if %ERRORLEVEL% EQU 0 (echo %ESC%[42m[ %1 Service Successfully Set to Manual ]%ESC%[0m
	) else (
		echo %1: %ESC%[41m[ Unexpected Error While Trying to Set to Manual %1 ]%ESC%[0m
	)
)
